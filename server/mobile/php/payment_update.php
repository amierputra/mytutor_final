...
$signed= hash_hmac('sha256', $signing, 'S-nM8MegCUgT45FM6DEVDBfg');
if ($signed === $data['x_signature']) {
    if ($paidstatus == "Success"){ //payment success
        $sqlinsertpayment = "INSERT INTO `tbl_payments`( `user_email`,`receipt_id`, `payment_paid`,`payment_status`) VALUES ('$email','$receiptid','$amount','$status')";
        $sqpupdatecart = "UPDATE `tbl_carts` SET `cart_status`='paid',`payment_id`='$receiptid' WHERE user_email='$email' AND cart_status IS NULL";
